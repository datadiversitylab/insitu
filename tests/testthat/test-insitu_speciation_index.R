...
test_that("insitu_speciation_index returns a data.frame", {
  ind <- insitu_speciation_index(phy = matched$phy, events = map_events)
  expect_s3_class(ind, "data.frame")
})

# Correct number of columns (4)
test_that("insitu_speciation_index returns a data.frame with 4 columns", {
  ind <- insitu_speciation_index(phy = matched$phy, events = map_events)
  expect_equal(ncol(ind), 4)
})
