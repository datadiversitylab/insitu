...
ASR. [`ape::ace()`](https://rdrr.io/pkg/ape/man/ace.html), which is used
in
[`insitu::run_geo_asr()`](https://datadiversitylab.github.io/insitu/reference/run_geo_asr.md),
is a traditional implementation of ASR. Stochastic character mapping
provides measures of uncertainty for estimated character state changes
by randomly sampling potential evolutionary histories across the
phylogeny.

In this vignette, we will use stochastic character mapping to estimate
the geographic history of a small sample of *Anolis* lizards native to
the Caribbean Islands.

First, let’s load and prepare the *Anolis* example data that we used in
...
Next, we will run the key function for estimating the geographic history
of 30 species of *Anolis* lizards that occur on the Caribbean Islands:
[`insitu::simmap_insitu()`](https://datadiversitylab.github.io/insitu/reference/simmap_insitu.md).
Once our data has been processed, we can use the
[`insitu::simmap_insitu()`](https://datadiversitylab.github.io/insitu/reference/simmap_insitu.md)
...
The function also requires the user to specify a transition model and a
number of simulations. The documentation for
[`ape::ace()`](https://rdrr.io/pkg/ape/man/ace.html) includes more
...
The `sim_results` object includes per-island ancestral state
probabilities for each node. As with the tradiational ASR approach
described in the Getting Started vignette, the `sim_results` object can
be used to map *in situ* speciation events and ultimately visualize
those events on the phylogeny.
...
![Figure 1. The example Anolis phylogeny with ancestral island states
visualized on the nodes.](events-1.png)

Figure 1. The example *Anolis* phylogeny with ancestral island states
visualized on the nodes.

Here, we visualize the colonization and *in situ* speciation history for
30 *Anolis* species across four islands. As discussed in the Getting
Started vignette, these events can be further contextualized using
several functions from `insitu`. One option is the
[`insitu::decompose_diversity()`](https://datadiversitylab.github.io/insitu/reference/decompose_diversity.md)
function, which provides users with counts of each type of potential
event (*in situ* speciation, export, and import; see the Getting Started
